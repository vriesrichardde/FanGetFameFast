...
Default deployment: runs locally in the devcontainer, reading/writing the
local cases directory (e.g. ~/cases). `scripts/setup_folder_structure.sh`
generates the registration below automatically in .claude/settings.json.
Note: this local INVESTIGATIONS_ROOT is independent of the
INVESTIGATIONS_SSH_HOST/INVESTIGATIONS_ROOT pair set via
./scripts/configure_vault.sh (the remote report-upload destination used by
lib/investigations_upload.py) — the two can point at different locations.

MCP Protocol: JSON-RPC 2.0 over stdio (MCP v2024-11-05)

Registration (.claude/settings.json) — local (default, auto-generated by
scripts/setup_folder_structure.sh):
  "investigations": {
    "command": "python3",
    "args": ["/absolute/path/to/mcp/investigations_server.py"],
    "env": { "INVESTIGATIONS_ROOT": "/home/vscode/cases" }
  }

Alternative — remote vault host: if the cases directory lives on a separate
host, register the server over SSH instead (see ./scripts/configure_vault.sh /
templates/set_env_template.sh — e.g. a SANS SIFT box, sansforensics@ubuntudesktop,
is one example deployment, not the only one). In this mode the server file is
deployed via SCP to $INVESTIGATIONS_ROOT/investigations_server.py on the remote
host, and mcp/investigations_server.py remains the canonical source. Keep this
registration in the gitignored .claude/settings.local.json, not the committed
settings.json:
  "investigations": {
    "command": "ssh",
    "args": [
