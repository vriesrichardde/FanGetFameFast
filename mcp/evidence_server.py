...
Default deployment: runs locally in the devcontainer, reading the local
evidence drop zone (e.g. ~/evidence). `scripts/setup_folder_structure.sh`
...
scripts/setup_folder_structure.sh):
  "evidence": {
    ...
    "env": { "EVIDENCE_ROOT": "/home/vscode/evidence" }
  }

Alternative — remote vault host: if the evidence lives on a separate host,
register the server over SSH instead (see ./scripts/configure_vault.sh /
...
deployed via SCP to $EVIDENCE_ROOT/evidence_server.py on the remote host, and
mcp/evidence_server.py remains the canonical source. Keep this registration in
the gitignored .claude/settings.local.json, not the committed settings.json:
  "evidence": {
    "command": "ssh",
    "args": [
